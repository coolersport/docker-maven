...
# Quick commands
    ...
    // usual maven command
    docker run -it --rm -v "$PWD":/src:z -w /src coolersport/maven:3.5.0-jdk-8 mvn clean install
    // with ability to build docker images
    docker run --privileged -it --rm -v /var/run:/var/run:z -v "$PWD":/src:z -w /src coolersport/maven:3.5.0-jdk-8 mvn clean install
